...
#ifndef __SESSIONM__
#define __SESSIONM__
#define __SESSIONM_SDK_VERSION__ @"2.3.1"
...
/*!
 ...
 @discussion If an in-app message or push notification was configured to have a deep link action, this method returns the deep link string that was setup in the SessionM Platform.
 @param sessionM SessionM service object.
 @param deepLink The deepLink string configured in the SessionM Platform portal.
 */
- (void)sessionM:(SessionM *)sessionM handleDeepLinkString:(NSString*)deepLink NS_SWIFT_NAME(sessionM(_:didReceiveDeepLinkString:));

/*!
 @abstract Notifies the developer that a SessionM remote notification was received.
 @discussion From this method, the developer can call <code>hasPendingNotificationAction</code> to see if a pending action is available and execute it with <code>executePendingNotificationAction</code>. The notification message can also be displayed to the user by calling <code>viewForMessage:</code>.
 @param sessionM SessionM service object.
 @param notification <code>SMNotificationMessage</code> object that represents the data sent in the notification.
 */
...
/*!
 ...
 @discussion In order to use this method, you must download the <code>SMPConfig.plist</code> configuration file for your app from the SessionM Platform portal, and then drag the file into Xcode to copy it to your app.
 ...
 This method should be called as early as possible in the application lifecycle, typically in <code>application:didFinishLaunchingWithOptions:</code> method of <code>UIApplicationDelegate</code>.
 */
- (void)startSessionFromConfigurationFile;
/*!
...
/*!
 ...
 @discussion This method should be called from the developer's <code>application:didReceiveRemoteNotification:</code> and <code>application:didFinishLaunchingWithOptions:</code> delegate methods with the respective <code>userInfo</code> or <code>launchOptions</code> payload. If the payload contains a SessionM notification, then an <code>SMNotificationMessage</code> object will be returned to the developer via the <code>sessionM:didReceiveNotification:</code> method.
...
/*!
 ...
 @abstract This notification is sent when the user's registration state is changed..
 */
